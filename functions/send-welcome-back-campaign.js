/**
 * Marketing campaign #1: "Welcome Back — Lingua Bud Has Been Growing!"
 * Same visual design as send-booking-campaign.js (header/body/footer, instructor
 * grid, free-trial banner) — only the messaging content changes for this send.
 *
 * Usage (run from the functions/ directory):
 *   RESEND_API_KEY=<key> GOOGLE_APPLICATION_CREDENTIALS=<path> node send-welcome-back-campaign.js --dry-run
 *   RESEND_API_KEY=<key> GOOGLE_APPLICATION_CREDENTIALS=<path> node send-welcome-back-campaign.js --render out.html
 *   RESEND_API_KEY=<key> GOOGLE_APPLICATION_CREDENTIALS=<path> node send-welcome-back-campaign.js --test
 *   RESEND_API_KEY=<key> GOOGLE_APPLICATION_CREDENTIALS=<path> node send-welcome-back-campaign.js --send --confirm
 *
 * Modes:
 *   --dry-run        Show recipient count, sample list, featured instructors. No emails sent.
 *   --render <path>  Write the rendered HTML (mock student, real featured instructors) to a
 *                     local file for review. No emails sent, no Resend call at all.
...
const SUBJECT            = 'Welcome Back — Lingua Bud Has Been Growing!';
...
// Instructor accounts to exclude from the featured cards (test/admin accounts).
// NOTE: Ian Jack's instructor profile uses linguabuddev@gmail.com, not his personal
// address — a previous campaign only excluded ianjack1643@gmail.com and missed it.
// Both emails are excluded here, plus a name-based backstop below, so this can't recur.
const EXCLUDED_INSTRUCTOR_EMAILS = new Set(['ianjack1643@gmail.com', 'linguabuddev@gmail.com']);
const EXCLUDED_INSTRUCTOR_NAMES  = new Set(['ian jack']);
...
const renderPath = renderIdx !== -1 ? args[renderIdx + 1] : null;

if (!isDryRun && !isTest && !isSend && !renderPath) {
  console.log(`
Lingua Bud — Welcome Back Campaign Sender
==========================================
Usage (from the functions/ directory):
  ...
  --render <path>
      Write the rendered HTML to a local file for review. No emails sent.
...
Environment variables required:
  RESEND_API_KEY                  Resend secret key (only needed for --test / --send)
  GOOGLE_APPLICATION_CREDENTIALS  Path to Firebase service account / ADC JSON
`);
  process.exit(0);
}

const resend = (isTest || isSend) ? new Resend(process.env.RESEND_API_KEY) : null;
...
async function fetchFeaturedInstructors() {
  ...
  const all  = snap.docs
    ...
    .filter(i => i.name
      && !EXCLUDED_INSTRUCTOR_EMAILS.has(i.email)
      && !EXCLUDED_INSTRUCTOR_NAMES.has(i.name.trim().toLowerCase()));
...
function buildInstructorCardHtml(instr) {
  ...
  const price      = formatPrice(instr.price_per_lesson, instr.currency);
  ...
  <tr>
    <td style="padding:20px 18px;text-align:center;">
      ...
      ${instr.native_country ? `<p style="margin:0 0 6px;font-size:12px;color:#888;">${instr.native_country}</p>` : ''}
      ...
      <tr>
        <td style="background:#20bcba;padding:36px 40px;text-align:center;">
          ...
          <h1 style="margin:0;color:#ffffff;font-size:28px;font-weight:700;line-height:1.3;">
            Welcome Back &mdash; Lingua Bud Has Been Growing!
          </h1>
          <p style="margin:10px 0 0;color:#e0fffe;font-size:15px;">
            New instructors, new languages, and a better lesson experience are waiting for you.
          </p>
        </td>
      </tr>
      ...
      <tr>
        <td style="background:#ffffff;padding:40px;">
          ...
          <p style="font-size:15px;color:#555;line-height:1.7;margin:0 0 28px;">
            It&rsquo;s been a while &mdash; and Lingua Bud has been busy. Our community has grown a lot
            since you last logged in, and we&rsquo;ve been working hard to make every lesson better.
          ...
          <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-bottom:10px;">
            <tr><td style="padding:14px 18px;background:#f0fffe;border-left:4px solid #20bcba;border-radius:0 8px 8px 0;">
              <strong style="color:#113448;font-size:14px;">&#127891; More instructors, more languages than ever</strong>
              <p style="margin:5px 0 0;color:#555;font-size:13px;line-height:1.6;">
                Our instructor community keeps growing, and so does the list of languages you can learn &mdash;
          ...
          <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-bottom:10px;">
            <tr><td style="padding:14px 18px;background:#f0fffe;border-left:4px solid #20bcba;border-radius:0 8px 8px 0;">
              <strong style="color:#113448;font-size:14px;">&#127909; Improved video chat, with screen sharing</strong>
              <p style="margin:5px 0 0;color:#555;font-size:13px;line-height:1.6;">
                We&rsquo;ve upgraded the video chat interface for smoother, clearer lessons. You and your
                instructor can now share screens right inside the call to work through documents,
                grammar exercises, or websites together in real time.
              </p>
            </td></tr>
          </table>
          ...
          <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-bottom:10px;">
            <tr><td style="padding:14px 18px;background:#f0fffe;border-left:4px solid #20bcba;border-radius:0 8px 8px 0;">
              <strong style="color:#113448;font-size:14px;">&#10024; A better lesson experience overall</strong>
              <p style="margin:5px 0 0;color:#555;font-size:13px;line-height:1.6;">
                From booking to homework assignments, we&rsquo;ve been refining the platform end to end
                so lessons feel smoother and more focused &mdash; for you and your instructor.
              </p>
            </td></tr>
          </table>

          <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-bottom:28px;">
            <tr><td style="padding:14px 18px;background:#fff8e1;border:1px solid #ffe082;border-radius:8px;">
              <strong style="color:#b45309;font-size:14px;">&#128241; A mobile app is in the works</strong>
              <p style="margin:5px 0 0;color:#555;font-size:13px;line-height:1.6;">
                We&rsquo;re currently building a Lingua Bud mobile app so you can connect and learn on the go.
                Stay tuned &mdash; we&rsquo;ll let you know the moment it&rsquo;s ready.
              </p>
            </td></tr>
          </table>

          <!-- Community growth blurb -->
          <p style="font-size:15px;color:#555;line-height:1.7;margin:0 0 28px;">
            We&rsquo;ve also been investing more in marketing to help the community grow &mdash; and it&rsquo;s
            working. <strong style="color:#113448;">New students are joining every week</strong>, which means
            more people to connect with, more instructors to choose from, and more energy in the community
            as a whole.
          </p>
            ...
            <tr>
              <td style="background:#113448;border-radius:10px;padding:28px 32px;text-align:center;">
                <p style="margin:0 0 8px;font-size:22px;font-weight:700;color:#ffffff;">
                  &#127881; Every Instructor Offers a Free Trial
                </p>
          ...
          <p style="font-size:15px;color:#555;line-height:1.7;margin:0 0 24px;">
            A lot has changed since you last visited &mdash; but the fastest way to see it for yourself is
            to book a free trial lesson. Pick an instructor above, or browse the full directory below.
          </p>

          <!-- Final CTA -->
          <div style="text-align:center;margin-bottom:4px;">
            <a href="https://linguabud.com/instructors"
               style="display:inline-block;background:#20bcba;color:#ffffff;padding:15px 40px;border-radius:6px;text-decoration:none;font-weight:700;font-size:16px;">
              Book Your Free Trial &rarr;
            </a>
          </div>
...
function buildPlainText(student, featured) {
  ...
  if (featured.length > 0) {
    ...
    for (const i of featured) {
      ...
      const price  = formatPrice(i.price_per_lesson, i.currency);
      const bio    = truncate(i.about_me, 150);
      const url    = `https://linguabud.com/instructor-profile?id=${i.id}`;
      instrBlock += `${i.name}${langs ? ' — ' + langs : ''}${i.native_country ? ' | ' + i.native_country : ''}\n`;
...
It's been a while — and Lingua Bud has been busy. Our community has grown a lot since you last
logged in, and we've been working hard to make every lesson better.

WHAT'S NEW
──────────────────────────────────────────────
• More instructors, more languages than ever
• Improved video chat interface, with screen sharing
• A better lesson experience overall
• A mobile app is currently in development
• We've increased our marketing efforts to grow the community
• New students are joining every week
...
${instrBlock}A lot has changed since you last visited — but the fastest way to see it for yourself is
...
async function main() {
  ...
  console.log(`  ✓ ${instructorPool.length} featured instructor candidates (approved, excluding test/admin accounts)`);
  console.log(`  ✓ ${students.length} eligible students (student role, not opted out)`);

  // ── Render to file (no network send) ─────────────────────────────────────
  if (renderPath) {
    const fs = require('fs');
    const mockStudent = {
      uid:               'preview-uid',
    ...
    console.log(`\n✓ Rendered HTML written to ${renderPath}`);
    console.log(`  Featured instructors (${featured.length}): ${featured.map(f => f.name).join(', ')}`);
    process.exit(0);
  }
  ...
  if (isDryRun) {
    ...
    console.log('\nRun with --render <path> to write the HTML to a file for review.');
    console.log('Run with --test to send a preview to your inbox.');
  ...
  if (isTest) {
    ...
    console.log(`  Featured instructors: ${featured.map(f => f.name).join(', ')}`);
