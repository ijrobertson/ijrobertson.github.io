...
import { getAuth, setPersistence, browserLocalPersistence, createUserWithEmailAndPassword, signInWithEmailAndPassword, signOut as _signOut, onAuthStateChanged as _onAuthStateChanged, deleteUser, sendPasswordResetEmail, verifyPasswordResetCode, confirmPasswordReset, connectAuthEmulator } from "https://www.gstatic.com/firebasejs/10.7.0/firebase-auth.js";
import { getFirestore, doc, getDoc, getDocFromServer, setDoc, deleteDoc, collection, collectionGroup, getDocs, addDoc, query, where, orderBy, limit, startAfter, onSnapshot, serverTimestamp, updateDoc, arrayUnion, arrayRemove, connectFirestoreEmulator, disableNetwork, enableNetwork } from "https://www.gstatic.com/firebasejs/10.7.0/firebase-firestore.js";
...
// ── Sign-out clears the home-screen icon badge ──────────────────────────────
//
// Every page that lets a user log out already imports `signOut` from this
// shared module (rather than duplicating sign-out logic — see js/app-shell.js's
// `lb-logout` event), so wrapping it here is the one place that guarantees
// the previous user's unread-message badge doesn't linger on the icon after
// logout, without needing an edit to every one of those pages. Cleared
// *before* the underlying signOut() call (not deferred to whatever page loads
// next) so it isn't racing the page's own post-signOut redirect.
async function signOut(authInstance) {
  try {
    ...
    // Badge failures are cosmetic — never worth blocking sign-out over.
  }
...
// ── Force a Firestore reconnect after the tab/app comes back from background ──
//
// Problem: on mobile (backgrounding the browser/PWA, locking the screen,
// switching wifi ↔ cellular), the WebChannel connection Firestore's SDK uses
// for onSnapshot listeners and in-flight writes can go stale without the SDK
// noticing — a send can sit "in flight" indefinitely, and a live listener
// stops receiving updates, until something forces a reconnect. This is why a
// chat message can look stuck (still showing "Sending...", or the sent text
// just not appearing) until the user manually leaves and reopens the
// conversation, which happens to force a fresh listener.
//
// Fix: cycle the network connection (drop + reconnect) whenever the page
// becomes visible again or the browser reports it's back online, so any
// writes/listeners silently stuck on a dead connection get flushed
// automatically instead of requiring a manual re-navigate.
...
async function reconnectFirestore() {
  ...
  try {
    ...
    // Non-critical — worst case the connection recovers on its own.
  } finally {
...
export {
  ...
  storage,
  functions,
  // Lets a page nudge a stuck operation (e.g. a chat send stalled well past
  // when it should have resolved) into recovering immediately, instead of
  // only recovering passively on the next visibilitychange/online event.
  reconnectFirestore,
  // Not constructed here (getMessaging(app) can throw in unsupported browsers/
  // insecure contexts) — js/push-notifications.js calls getMessaging(app) itself,
  // lazily and inside a try/catch, so an unsupported browser only fails gracefully
  // within the push flow rather than breaking this shared, universally-imported module.
  getMessaging,
  getToken,
