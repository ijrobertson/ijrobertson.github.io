...
//
// A 3-second timeout races against authStateReady() so that if IndexedDB
// is slow or unresponsive (seen on iOS Safari under memory pressure) the
// auth listener still registers and the page is never stuck loading forever.
//
...
function onAuthStateChanged(authInstance, callback) {
  ...
  // Race against a timeout so a hung IndexedDB read never blocks pages indefinitely.
  let readyPromise;
  try {
    const readyOrTimeout = Promise.race([
      (typeof authInstance.authStateReady === 'function')
        ? authInstance.authStateReady()
        : Promise.resolve(),
      new Promise(resolve => setTimeout(resolve, AUTH_READY_TIMEOUT_MS))
    ]);
    readyPromise = readyOrTimeout;
  } catch(e) {
